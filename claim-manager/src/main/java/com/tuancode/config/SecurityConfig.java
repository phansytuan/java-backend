...
import com.tuancode.security.CustomAuthenticationSuccessHandler;
import com.tuancode.security.filter.JwtAuthenticationFilter;
...
public class SecurityConfig {
  ...
  // config cho spring security tiếp nhận class CustomAuthenticationSuccessHandler xử lý sau khi login thành công
  @Autowired
        ...
        .formLogin( // custom form login, không sử dụng form login mặc định của spring security
            form ->
                form.loginPage("/login")// page login được custom
                    ...
                    .successHandler(customAuthenticationSuccessHandler) // config xử lý trong class customAuthenticationSuccessHandler khi login thành công
//                    .defaultSuccessUrl("/process-after-login-success",true) // /process-after-login-success url được điều hướng đến khi login thành công, sử dụng để xử lý tự động điều hướng page theo role, ví dụ role Admin -> /cms/home, role user -> /home
        ...
        .logout(logout -> logout.logoutRequestMatcher(new AntPathRequestMatcher("/logout"))
            .deleteCookies(Constant.JWT.JWT.name()) // xoá bỏ cookie lưu token jwt này sau khi đã logout
            .permitAll());
    // để spring security bắt buộc chạy qua tầng filter này trước khi đến controller
  ...
  public static void main(String[] args) {
    ...
    System.out.println("secret key jwt = " + generateSafeToken());
  }
